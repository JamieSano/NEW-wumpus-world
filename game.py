...
class WumpusWorld:
    ...
    def prepare_environment(self):
        self.g_w_p_coords = self.random_pos_gwp()
        chars = ['G', 'W', 'P', 'P', 'P']
        ...
        func.print_world_console(self.world)
    ...
    def move_agent(self, x, y):
        ...
        func.print_world_console(self.agent.kb.inference)
        self.find_agent()
    ...
    def game_status(self):
        for i, pos in enumerate(self.g_w_p_coords):
            if self.agent.location == pos: # if the agent is in either gold, wumpus, or pits locations
                return i
        else:
    ...
    def is_wumpus_killed(self, direction):
        ...
        if direction == 'U' or direction == 'D':
            if func.check_coords(self.agent.location, wumpus_xy, 'C'):
            ...
            if func.check_coords(self.agent.location, wumpus_xy, 'R'):
                self.world = func.remove_char(wumpus_xy[0], wumpus_xy[1], 'W', self.world[:])
                self.agent.w_killed = True
...
class Agent:
    ...
    def check_breeze_pattern(self, pattern):
        if len(pattern["pattern"]) == 3:
            if all(self.kb.world_info[coord[0]][coord[1]].get("Breeze") for coord in pattern["pattern"]):                
                row, col = pattern["location"]
                self.pit_located(row, col, True)
        ...
        for x in range(WORLD_SIZE):
            for y in range(WORLD_SIZE):
                if self.kb.world_info[x][y]:
                    if all(value is None or key in ['Bump', 'Scream', 'Breeze'] for key, value in self.kb.world_info[x][y].items()):
                        self.kb.inference[x][y] = ''
                    elif 'Breeze' in self.kb.world_info[x][y]:
                        self.unsafe.append((x, y))
    ...
    def normal_move(self):
        ...
        for x, y in valid_adj_cells:
            ...
            if safety == -1:
                continue
            elif safety == 0 and self.predict_unsafe(x, y):  
                continue                                     
    ...
    def is_move_safe(self, x, y):
        if not self.kb.world_info[x][y]:   
            return 0
        elif all(value is None or key in ['Bump', 'Scream'] for key, value in self.kb.world_info[x][y].items()):  
...
class Knowledge:
    ...
    def add(self, pos, sensors):
        ...
        print("KB of World")
        self.print_world_console_info()


    def print_world_console_info(self):
        for i in range(4):
            print("+" + "-" * 35 + "+" + "-" * 35 + "+"+ "-" * 35 + "+"+ "-" * 35 + "+")
            for j in range(4):
